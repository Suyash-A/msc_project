...
def compute_visualchexbert_f1_scores(true_labels_path, pred_labels_dir, test_report_path):
    ...
    for file_name in os.listdir(pred_labels_dir):
        if file_name.startswith("visualchexbert_labeled_") and file_name.endswith(".csv"):
            ...
            print("#" * 80)
            visualchexbert_df = pd.read_csv(os.path.join(pred_labels_dir, file_name))

            visualchexbert_df = visualchexbert_df.merge(
                test_reports_df, how='inner', left_on='Report Impression', right_on='text'
            )

            visualchexbert_df.drop_duplicates(inplace=True)

            visualchexbert_df['id'] = visualchexbert_df['id'].astype(str).str.lstrip('s')
            visualchexbert_df.set_index('id', inplace=True)
            visualchexbert_df.rename(columns={'Airspace Opacity': 'Lung Opacity'}, inplace=True)
            visualchexbert_df = visualchexbert_df[categories]
            ...
            common_ids = true_df.index.intersection(visualchexbert_df.index)
            true_df = true_df.loc[common_ids]
            visualchexbert_df = visualchexbert_df.loc[common_ids]

            print('VisualCheXbert: Presence')
            df, preds, targets = evaluate_labels(true_df, visualchexbert_df, method='presence')
            df.columns = pd.MultiIndex.from_tuples([('VisualCheXbert', c) for c in df.columns])
...
if __name__ == "__main__":
    ...
    compute_visualchexbert_f1_scores(args.true_labels, args.pred_labels_dir, args.test_report_path)
