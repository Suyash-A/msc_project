...
def compute_chexpert_f1_scores(true_labels_path, pred_labels_dir, test_report_path):
    ...
    for file_name in os.listdir(pred_labels_dir):
        if file_name.startswith("chexpert_labeled_") and file_name.endswith(".csv"):
            print("#" * 80)
            print(f"Processing {file_name}...")
            print("#" * 80)
            chexpert_df = pd.read_csv(os.path.join(pred_labels_dir, file_name))

            chexpert_df = chexpert_df.merge(
                test_reports_df, how='inner', left_on='Reports', right_on='text'
            )

            chexpert_df.drop_duplicates(inplace=True)

            chexpert_df['id'] = chexpert_df['id'].astype(str).str.lstrip('s')
            chexpert_df.set_index('id', inplace=True)
            chexpert_df.rename(columns={'Airspace Opacity': 'Lung Opacity'}, inplace=True)
            chexpert_df = chexpert_df[categories]
            ...
            common_ids = true_df.index.intersection(chexpert_df.index)
            true_df = true_df.loc[common_ids]
            chexpert_df = chexpert_df.loc[common_ids]

            print('CheXpert: Mention')
            df, preds, targets = evaluate_labels(true_df, chexpert_df, method='mention')
            ...
            print('CheXpert: Uncertain')
            df, preds, targets = evaluate_labels(true_df, chexpert_df, method='uncertain')
            df.columns = pd.MultiIndex.from_tuples([('CheXpert', c) for c in df.columns])
            ...
            print('CheXpert: Absence')
            df, preds, targets = evaluate_labels(true_df, chexpert_df, method='absence')
            df.columns = pd.MultiIndex.from_tuples([('CheXpert', c) for c in df.columns])
            df.columns = df.columns.reorder_levels([1, 0])
            df = df[['precision', 'recall', 'f1', 'positives']]
            for c in df.columns:
            ...
            print('CheXpert: Presence')
            df, preds, targets = evaluate_labels(true_df, chexpert_df, method='presence')
            df.columns = pd.MultiIndex.from_tuples([('CheXpert', c) for c in df.columns])
            df.columns = df.columns.reorder_levels([1, 0])
            df = df[['precision', 'recall', 'f1', 'positives']]
            for c in df.columns:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process CheXpert predictions and compute F1 scores.")
    ...
    compute_chexpert_f1_scores(args.true_labels, args.pred_labels_dir, args.test_report_path)
