...
def process_and_compute_f1_scores(true_labels_path, pred_labels_dir, ordered_test_ids_path):
    ...
    for file_name in os.listdir(pred_labels_dir):
        if file_name.startswith("chexpert_labeled_") and file_name.endswith(".csv"):
            ...
            chexpert_df = chexpert_df.drop(columns=['Reports'])

            # Rename the 'Lung Opacity' column to 'Airspace Opacity'
            chexpert_df = chexpert_df.rename(columns={'Lung Opacity': 'Airspace Opacity'})

            # Concatenate the ordered_test_ids with the chexpert dataframe (excluding the 'Reports' column)
            result_df = pd.concat([ordered_test_ids_df, chexpert_df], axis=1)
            ...
            micro_f1 = f1_score(y_true_all, y_pred_all, labels=[-1, 0, 1], average='micro', zero_division=0)
            macro_f1 = f1_score(y_true_all, y_pred_all, labels=[-1, 0, 1], average='macro', zero_division=0)
            weighted_f1 = f1_score(y_true_all, y_pred_all, labels=[-1, 0, 1], average='weighted', zero_division=0)
